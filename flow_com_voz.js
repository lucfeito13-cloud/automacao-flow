...
// Versão 4.0 - Original + Flow Voz Completo
// ==========================================
//
...
//   - Referências de imagem usam [Nome]
//   - Vozes usam <voz: Nome>
//
(function() {
    'use strict';
    ...
    const CONFIG = {
        ...
        REF_SUFFIX: ' _',
        VERSION: 'Flow Voz (Original Completo)',
    };
    ...
    function parsePrompt(prompt) {
        const segs = [];
        // Agora busca referências em [colchetes] OU vozes em <voz: Nome>
        const re = /(\[([^\]]+)\]|<voz:\s*([^>]+)>)/gi;
        let last = 0, m;
  ...
  <div class="flow-assign-reload-bar" id="flow-assign-reload-bar"><button id="flow-assign-reload">🔄 Atualizar Página</button></div>
    ...
    class FlowAutomation {
        ...
        constructor() {
            ...
            this.setupDragDrop();
            log.success('Flow Automation (Flow Voz) inicializado!');
        ...
        async clickSubmit() {
            ...
            for (let i = 0; i < 30; i++) { if (!btn.disabled) break;
                await this.dynamicSleep(CONFIG.DELAY_SHORT); }
        ...
        async apiRename(workflowId, newName) {
            if (!_authToken) { this.logDebug('Token não capturado', 'error'); return false; }
        ...
        async validateReferences(source = 'images') {
            ...
            try {
                ...
                const prompts = parsePromptsText(text);
                const refs = extractReferences(prompts);
                if (!refs.length) { this.validatedRefs = {}; updateFn(); btn.disabled = false; btn.textContent = '🔍 Validar referências'; return; }
                ...
                updateFn();
                if (!pending.size) statusFn('success', `✅ Todas as referências encontradas!`);
        ...
        async getTileName(tile) {
            ...
            const UI = ['favorite','redo','more_vert','image','warning','refresh','delete_forever','undo','play_arrow','pause','download', 'Adicionar aos favoritos','Reutilizar comando','Mais','Add to favorites','Reuse prompt','More','Falha','Ops!','Tentar novamente','Excluir','Failed','Oops!','Retry','Delete'];
            let nome = null;
            for (let t = 0; t < 5; t++) {
        ...
        // ============================================================
        // PAINEL DE ATRIBUIÇÃO E DRAG & DROP
        // ============================================================
        ...
        showAssignPanel(allMatrices) {
            ...
            if (this.genMode === 'refs') {
                for (const name of this.refNames) {
                    ...
                    item.className = 'flow-assign-item'; item.draggable = true; item.dataset.type = 'ref'; item.dataset.name = name;
                    item.innerHTML = `<span class="drag-icon">⋮</span><span class="assign-name">${this.esc(name)}</span><span class="assign-status">⏳</span>`;
                    item.addEventListener('dragstart', e => { e.dataTransfer.setData('text/plain', JSON.stringify({ type: 'ref', name })); e.dataTransfer.effectAllowed = 'copy'; });
                ...
                for (let i = 1; i <= this.sceneCount; i++) {
                    ...
                    const item = document.createElement('div');
                    item.className = 'flow-assign-item'; item.draggable = true; item.dataset.type = 'scene'; item.dataset.scene = sceneName; item.dataset.sceneNum = i;
                    item.innerHTML = `<span class="drag-icon">⋮</span><span class="assign-name">${sceneName}</span><span class="assign-status">⏳</span>`;
                    item.addEventListener('dragstart', e => { e.dataTransfer.setData('text/plain', JSON.stringify({ type: 'scene', sceneNum: i, sceneName })); e.dataTransfer.effectAllowed = 'copy'; });
                    items.appendChild(item);
                }
            }
            panel.classList.add('active'); panel.classList.remove('minimized');
        ...
        showVideoAssignPanel(allMatrices) {
            ...
            for (const [sceneName] of this.videoSceneAssignments) {
                ...
                const item = document.createElement('div');
                item.className = 'flow-assign-item'; item.draggable = true; item.dataset.type = 'scene'; item.dataset.scene = sceneName; item.dataset.sceneNum = sceneNum;
                item.innerHTML = `<span class="drag-icon">⋮</span><span class="assign-name">${sceneName}</span><span class="assign-status">⏳</span>`;
            ...
            panel.classList.add('active'); panel.classList.remove('minimized');
            this._videoAssignActive = true; this.updateAssignCount();
        }

        hideAssignPanel() { document.getElementById('flow-assign-panel').classList.remove('active'); }
        reopenAssignPanel() { document.getElementById('flow-assign-panel').classList.add('active'); }
        toggleAssignPanel() { document.getElementById('flow-assign-panel').classList.toggle('minimized'); }

        updateAssignCount() {
            const el = document.getElementById('flow-assign-count');
            if (this.genMode === 'refs' && !this._videoAssignActive) {
                const total = this.refNames.length; const done = [...this.refAssignments.values()].filter(Boolean).length;
                el.textContent = `${done}/${total}`;
            } else if (this._videoAssignActive) {
                const total = this.videoSceneAssignments.size; const done = [...this.videoSceneAssignments.values()].filter(arr => arr.length > 0).length;
                el.textContent = `${done}/${total}`;
            } else if (this.genMode === 'scenes') {
                const total = this.sceneCount; const done = [...this.sceneAssignments.values()].filter(arr => arr.length > 0).length;
                el.textContent = `${done}/${total}`;
            }
        }
        ...
        setupDragDrop() {
            document.addEventListener('dragover', e => {
                ...
                if (tile) { e.preventDefault(); e.dataTransfer.dropEffect = 'copy'; const inner = tile.querySelector('[data-tile-id]') || tile; document.querySelectorAll('.drop-hover').forEach(el => el.classList.remove('drop-hover')); inner.classList.add('drop-hover'); }
            });
            document.addEventListener('dragleave', e => {
                const related = e.relatedTarget?.closest('[data-tile-id]'); const current = e.target.closest('[data-tile-id]');
                if (current && current !== related) current.classList.remove('drop-hover');
            });
            document.addEventListener('drop', async e => {
                const tile = e.target.closest('[data-tile-id]'); if (tile) tile.classList.remove('drop-hover'); if (!tile) return;
            ...
            document.addEventListener('click', async e => {
                ...
                await this.apiRename(wfId, 'Imagem gerada'); await this.apiFavorite(wfId, false); label.remove();
                if (type === 'ref') { const name = label.dataset.name; this.refAssignments.delete(name); this.updateAssignItemUI(name, false); } 
                else if (type === 'scene') {
                    const sceneName = label.dataset.scene; const arr = this.sceneAssignments.get(sceneName);
                ...
                this.tileAssignments.delete(wfId); this.updateAssignCount();
            });
        }

        async assignReference(name, workflowId, tileEl) {
            const prevWfId = this.refAssignments.get(name);
            if (prevWfId && prevWfId !== workflowId) { await this.apiRename(prevWfId, 'Imagem gerada'); await this.apiFavorite(prevWfId, false); this.removeLabelFromTile(prevWfId); this.tileAssignments.delete(prevWfId); }
            const prevAssign = this.tileAssignments.get(workflowId);
            if (prevAssign) { if (prevAssign.type === 'ref') this.refAssignments.delete(prevAssign.name); this.removeLabelFromTile(workflowId); }
            const apiName = name + CONFIG.REF_SUFFIX;
            if (await this.apiRename(workflowId, apiName) && await this.apiFavorite(workflowId, true)) {
                this.refAssignments.set(name, workflowId); this.tileAssignments.set(workflowId, { label: name, type: 'ref', name }); this.addLabelToTile(tileEl, name, workflowId, 'ref', name); this.updateAssignItemUI(name, true); this.updateAssignCount(); this.startLabelObserver();
            }
        }
        ...
        async assignScene(sceneNum, sceneName, workflowId, tileEl) {
            ...
            const prevAssign = this.tileAssignments.get(workflowId);
            if (prevAssign) {
                if (prevAssign.type === 'scene') { const prevArr = assignments.get(prevAssign.scene); if (prevArr) { const idx = prevArr.findIndex(a => a.workflowId === workflowId); if (idx >= 0) prevArr.splice(idx, 1); } this.updateAssignItemUI(prevAssign.scene, (prevArr?.length || 0) > 0); } 
                else if (prevAssign.type === 'ref') { this.refAssignments.delete(prevAssign.name); this.updateAssignItemUI(prevAssign.name, false); }
                this.removeLabelFromTile(workflowId);
            }
            const imgNum = arr.length + 1; const fullName = `Cena ${sceneNum} - ${itemLabel} ${imgNum}`;
            if (await this.apiRename(workflowId, fullName) && await this.apiFavorite(workflowId, true)) {
                arr.push({ imgNum, workflowId, src: this.getImgSrcFromTile(tileEl) || '' }); assignments.set(sceneName, arr); this.tileAssignments.set(workflowId, { label: fullName, type: 'scene', scene: sceneName, imgNum }); this.addLabelToTile(tileEl, fullName, workflowId, 'scene', sceneName); this.updateAssignItemUI(sceneName, true); this.updateAssignCount(); this.startLabelObserver();
            }
        }

        addLabelToTile(tileEl, text, workflowId, type, extraData) {
            this.removeLabelFromTile(workflowId);
            const outer = tileEl.closest('[data-tile-id]') || tileEl; outer.style.position = 'relative';
            const label = document.createElement('div'); label.className = 'flow-tile-label'; label.dataset.wf = workflowId; label.dataset.type = type;
            if (type === 'ref') label.dataset.name = extraData; if (type === 'scene') label.dataset.scene = extraData;
            label.innerHTML = `<span>${this.esc(text)}</span><button class="label-x">×</button>`; outer.appendChild(label);
        }

        removeLabelFromTile(workflowId) { document.querySelectorAll(`.flow-tile-label[data-wf="${workflowId}"]`).forEach(l => l.remove()); }
        ...
        startLabelObserver() {
            ...
            this._labelObserverId = setInterval(() => {
                ...
                document.querySelectorAll('a[href*="/edit/"]').forEach(link => {
                    const m = link.href.match(/\/edit\/([a-f0-9-]{36})/); if (!m) return;
                    const wfId = m[1]; const data = this.tileAssignments.get(wfId); if (!data) return;
                    const tile = link.closest('[data-tile-id]'); if (!tile || tile.querySelector(`.flow-tile-label[data-wf="${wfId}"]`)) return;
                    tile.style.position = 'relative'; const label = document.createElement('div'); label.className = 'flow-tile-label'; label.dataset.wf = wfId; label.dataset.type = data.type;
                    if (data.type === 'ref') label.dataset.name = data.name || ''; if (data.type === 'scene') label.dataset.scene = data.scene || '';
                    label.innerHTML = `<span>${this.esc(data.label)}</span><button class="label-x">×</button>`; tile.appendChild(label);
                });
            }, 600);
        }
        ...
        updateAssignItemUI(name, assigned) {
            document.querySelectorAll('.flow-assign-item').forEach(item => {
                if ((item.dataset.name || item.dataset.scene) === name) {
                    item.classList.toggle('assigned', assigned);
                    const status = item.querySelector('.assign-status'); if (status) status.textContent = assigned ? '✅' : '⏳';
                }
            });
        }

        async downloadScenes() {
            const btn = document.getElementById('flow-assign-download'); btn.disabled = true; btn.textContent = '⏳ Baixando...';
            const assignments = this._videoAssignActive ? this.videoSceneAssignments : this.sceneAssignments;
            const ext = this._videoAssignActive ? 'mp4' : 'jpg';
            for (const [sceneName, imgs] of [...assignments.entries()].sort((a,b) => parseInt(a[0].match(/\d+/)?.[0]||0) - parseInt(b[0].match(/\d+/)?.[0]||0))) {
                const sceneNum = parseInt(sceneName.match(/\d+/)?.[0] || 0); const sorted = imgs.sort((a,b) => a.imgNum - b.imgNum);
                for (let i = 0; i < sorted.length; i++) {
                    const fileName = i === 0 ? `cena_${sceneNum}.${ext}` : `cena_${sceneNum}_${i+1}.${ext}`;
                    let src = sorted[i].src;
                    if (!src) { const link = document.querySelector(`a[href*="/edit/${sorted[i].workflowId}"]`); if (link) { const tile = link.closest('[data-tile-id]'); if (tile) src = this.getMediaSrcFromTile(tile); } }
                    if (!src) continue;
                    try { const resp = await _origFetch(src); const blob = await resp.blob(); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = fileName; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); await this.sleep(400); } catch(e) {}
                }
            }
        ...
        async downloadProjectImages(mode) {
            ...
            const btn = document.getElementById(btnId); const origText = btn?.textContent; if (btn) { btn.disabled = true; btn.textContent = '⏳ Baixando...'; }
            try {
                if (mode === 'all') {
                    const scroller = this.getScroller(); if (!scroller) return; scroller.scrollTop = 0; await this.sleep(600);
                    let count = 0; let downloaded = new Set();
                    for (let iter = 0; iter < 500; iter++) {
                        const tiles = [...document.querySelectorAll('[data-tile-id]')].filter(el => el.parentElement.closest('[data-tile-id]'));
                        for (const tile of tiles) {
                            if (!this.isTileLoaded(tile)) continue; const uuid = this.getUuidFromTile(tile); if (!uuid || downloaded.has(uuid)) continue; downloaded.add(uuid);
                            const mediaSrc = this.getMediaSrcFromTile(tile); if (!mediaSrc) continue; const tileIsVideo = this.isVideoTile(tile);
                            const link = tile.querySelector('a[href*="/edit/"]'); const wfId = link?.href.match(/\/edit\/([a-f0-9-]{36})/)?.[1]; const data = wfId ? this.tileAssignments.get(wfId) : null;
                            let fileName;
                            if (data?.type === 'scene') { const m = data.label.match(/Cena\s+(\d+)\s*-\s*(?:Imagem|Vídeo|Video)\s+(\d+)/i); const ext = tileIsVideo ? 'mp4' : 'jpg'; if (m) fileName = parseInt(m[2]) === 1 ? `cena_${m[1]}.${ext}` : `cena_${m[1]}_${m[2]}.${ext}`; else fileName = `cena_${data.label.replace(/\s+/g, '_')}.${ext}`; } 
                            else if (data?.type === 'ref') { const clean = (data.name || data.label).replace(/ _$/, '').trim(); fileName = `referencia_${clean.toLowerCase().replace(/\s+/g, '_')}.jpg`; } 
                            else { const ext = tileIsVideo ? 'mp4' : 'jpg'; fileName = `media_${count + 1}.${ext}`; }
                            try { const resp = await _origFetch(mediaSrc); const blob = await resp.blob(); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = fileName; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); count++; await this.sleep(300); } catch(e) {}
                        }
                        const prev = scroller.scrollTop; scroller.scrollTop += 350; await this.sleep(400); if (scroller.scrollTop === prev) break;
                    }
                    scroller.scrollTop = 0;
                } else {
                    const entries = [...this.tileAssignments.entries()].filter(([, data]) => { if (mode === 'identified') return true; if (mode === 'scenes') return data.type === 'scene'; if (mode === 'refs') return data.type === 'ref'; return false; });
                    if (!entries.length) { if (btn) { btn.disabled = false; btn.textContent = origText; } return; }
                    let count = 0; const pending = new Map(entries); const scroller = this.getScroller();
                    if (scroller) {
                        scroller.scrollTop = 0; await this.sleep(600);
                        for (let iter = 0; iter < 500 && pending.size > 0; iter++) {
                            document.querySelectorAll('a[href*="/edit/"]').forEach(async link => {
                                const m = link.href.match(/\/edit\/([a-f0-9-]{36})/); if (!m) return; const wfId = m[1]; const data = pending.get(wfId); if (!data) return;
                                const tile = link.closest('[data-tile-id]'); if (!tile || !this.isTileLoaded(tile)) return; const mediaSrc = this.getMediaSrcFromTile(tile); if (!mediaSrc) return; const tileIsVideo = this.isVideoTile(tile);
                                let fileName;
                                if (data.type === 'scene') { const sm = data.label.match(/Cena\s+(\d+)\s*-\s*(?:Imagem|Vídeo|Video)\s+(\d+)/i); const ext = tileIsVideo ? 'mp4' : 'jpg'; if (sm) fileName = parseInt(sm[2]) === 1 ? `cena_${sm[1]}.${ext}` : `cena_${sm[1]}_${sm[2]}.${ext}`; else fileName = `cena_${data.label.replace(/\s+/g, '_')}.${ext}`; } 
                                else if (data.type === 'ref') { const clean = (data.name || data.label).replace(/ _$/, '').trim(); fileName = `referencia_${clean.toLowerCase().replace(/\s+/g, '_')}.jpg`; } 
                                else { const ext = tileIsVideo ? 'mp4' : 'jpg'; fileName = `media_${wfId.substring(0, 8)}.${ext}`; }
                                try { const resp = await _origFetch(mediaSrc); const blob = await resp.blob(); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = fileName; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); count++; pending.delete(wfId); await this.sleep(400); } catch(e) {}
                            });
                            if (pending.size === 0) break; const prev = scroller.scrollTop; scroller.scrollTop += 350; await this.sleep(400); if (scroller.scrollTop === prev) break;
                        }
                        scroller.scrollTop = 0;
                    }
                }
        ...
        async analyzeProject(source = 'images') {
            ...
            const statusFn = isVideo ? (t, m) => this.setVideoStatus(t, m) : (t, m) => this.setStatus(t, m);
            const btn = document.getElementById(btnId); btn.disabled = true; btn.textContent = '⏳ Analisando...';
            document.querySelectorAll('.flow-tile-label').forEach(l => l.remove()); this.tileAssignments.clear();
            const scroller = this.getScroller(); if (!scroller) { btn.disabled = false; btn.textContent = '🔍 Analisar projeto existente'; return; }
            let labelsFound = 0; const checkedIds = new Set();
            scroller.scrollTop = 0; await this.sleep(600);
            for (let iter = 0; iter < 300; iter++) {
                const tiles = [...document.querySelectorAll('[data-tile-id]')].filter(el => el.parentElement.closest('[data-tile-id]'));
                for (const tile of tiles) {
                    const tileId = tile.getAttribute('data-tile-id'); if (checkedIds.has(tileId)) continue; checkedIds.add(tileId);
                    const name = await this.getTileName(tile); if (!name) continue;
                    const wfId = this.getWorkflowIdFromTile(tile); let labelText = null, type = null, extra = null;
                    const sceneMatch = name.match(/^Cena\s+(\d+)\s*-\s*(?:Imagem|Vídeo|Video)\s+(\d+)$/i);
                    if (sceneMatch) { labelText = name; type = 'scene'; extra = `Cena ${sceneMatch[1]}`; }
                    else if (name.endsWith(CONFIG.REF_SUFFIX)) { const cleanName = name.slice(0, -CONFIG.REF_SUFFIX.length); labelText = cleanName; type = 'ref'; extra = cleanName; }
                    if (labelText && type && wfId) { const outer = tile.closest('[data-tile-id]') || tile; this.addLabelToTile(outer, labelText, wfId, type, extra); this.tileAssignments.set(wfId, { label: labelText, type, name: extra, scene: extra }); labelsFound++; }
                }
                const prev = scroller.scrollTop; scroller.scrollTop += 350; await this.sleep(400); if (scroller.scrollTop === prev) break;
            }
        ...
        // ============================================================
        // UI HELPERS (Status, Progress, etc)
        // ============================================================
        ...
        updateProgress(fraction) { const pct = Math.round(fraction * 100); document.getElementById('flow-progress-bar').style.width = pct + '%'; document.getElementById('flow-mini-progress-bar').style.width = pct + '%'; }
        ...
        updatePromptItemStatus(index, status, extra = '') {
            const item = document.querySelector(`.flow-prompt-item[data-index="${index}"]`); if (!item) return;
            item.className = `flow-prompt-item ${status}`; let badge = item.querySelector('.status-badge');
            const icons  = { active:'⚡', done:'✅', error:'❌', retrying:'🔄' }; const labels = { active:'Gerando', done:'Concluído', error:'Falhou', retrying:'Retentando' };
            const colors = { active:{ bg:'#e0f2fe', fg:'#0369a1' }, done:{ bg:'#d1fae5', fg:'#065f46' }, error:{ bg:'#fee2e2', fg:'#991b1b' }, retrying:{ bg:'#fef9c3', fg:'#78350f' } };
            if (status !== 'active' || extra) { if (!badge) { badge = document.createElement('span'); badge.className = 'status-badge'; item.appendChild(badge); } badge.textContent = `${icons[status] || ''} ${extra || labels[status] || status}`; const clr = colors[status] || colors.active; badge.style.background = clr.bg; badge.style.color = clr.fg; } else if (badge) { badge.remove(); }
        }
        updateVideoPromptItemStatus(index, status, extra = '') {
            const item = document.querySelector(`#fv-prompt-list .flow-prompt-item[data-index="${index}"]`); if (!item) return;
            item.className = `flow-prompt-item ${status}`; let badge = item.querySelector('.status-badge');
            const icons  = { active:'⚡', done:'✅', error:'❌', retrying:'🔄' }; const labels = { active:'Gerando', done:'Concluído', error:'Falhou', retrying:'Retentando' };
            const colors = { active:{ bg:'#e0f2fe', fg:'#0369a1' }, done:{ bg:'#d1fae5', fg:'#065f46' }, error:{ bg:'#fee2e2', fg:'#991b1b' }, retrying:{ bg:'#fef9c3', fg:'#78350f' } };
            if (status !== 'active' || extra) { if (!badge) { badge = document.createElement('span'); badge.className = 'status-badge'; item.appendChild(badge); } badge.textContent = `${icons[status] || ''} ${extra || labels[status] || status}`; const clr = colors[status] || colors.active; badge.style.background = clr.bg; badge.style.color = clr.fg; } else if (badge) { badge.remove(); }
