  ...
  <div class="flow-header">
    <div class="flow-header-left">
      ...
      <div>
        ...
        <div class="flow-header-subtitle">Flow</div>
      </div>
    </div>
  ...
  <div class="flow-assign-reload-bar" id="flow-assign-reload-bar"><button id="flow-assign-reload">🔄 Atualizar Página</button></div>
    ...
    class FlowAutomation {
        ...
        async validateReferences(source = 'images') {
            ...
            btn.disabled = true;
            btn.textContent = '⏳ Escaneando galeria...';
            try {
                const text = document.getElementById(inputId).value;
                const prompts = parsePromptsText(text);
                const refs = extractReferences(prompts);
                if (!refs.length) { this.validatedRefs = {}; updateFn(); btn.disabled = false;
                btn.textContent = '🔍 Validar referências na galeria'; return; }
        ...
        async downloadScenes() {
            ...
            btn.disabled = false;
            btn.textContent = '⬇️ Baixar Cenas';
        }
        ...
        async downloadProjectImages(mode) {
            const btnId = { identified: 'flow-dl-identified', scenes: 'flow-dl-scenes', refs: 'flow-dl-refs', all: 'flow-dl-all' }[mode] || 
                          { identified: 'fv-dl-identified', scenes: 'fv-dl-scenes', all: 'fv-dl-all' }[mode];
            const btn = document.getElementById(btnId);
        ...
        async analyzeProject(source = 'images') {
            ...
            const scroller = this.getScroller();
            if (!scroller) { btn.disabled = false;
            btn.textContent = '🔍 Analisar projeto existente'; return; }
            ...
            btn.disabled = false;
            btn.textContent = '🔍 Analisar projeto existente';
        }
            ...
            try {
                ...
                for (let bIdx = 0; bIdx < batches.length; bIdx++) {
                    ...
                    this.updateMini(
                        ...
                        batch.map(p => `#${p.promptNum}`).join(' + '),
                        bIdx / batches.length,
                        `${this.videoGenMode === 'scenes' ? 'Cenas' : this.videoGenMode === 'voice' ? 'Vozes' : 'Livre'} • ${N} resultado(s)/prompt • ${this.videoBatchSize} simult.`
                    );
                    this.logVideoDebug(`\n╭─── LOTE ${bIdx+1}/${batches.length}: prompts ${batch.map(p=>p.promptNum).join(', ')} ───╮`, 'info');
